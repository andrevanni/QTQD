...
- **Service Worker:** versão atual `qtqd-v12` em `frontend_cliente/sw.js`. Ao fazer mudanças que devem invalidar cache, incrementar a versão.
...
QTQD/
  frontend_cliente/       Portal do cliente
    ...
    sw.js                 Service Worker (cache qtqd-v12)
...
Colunas (qualquer ordem, header na linha 1): `Nome Completo`, `Linha`, `Curva` (A/B/C/D), `Filial` (1-6), `MediaF Un` (média mensal de venda em unidades), `Qtd Estoque`, `Estoque Valor`.

#### Regra de cálculo
Cada produto é agregado por `(nome, linha, curva)` somando todas as filiais. Para cada produto agregado:
