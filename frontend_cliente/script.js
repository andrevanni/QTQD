...
(function(){const p=new URLSearchParams(location.search);const token=p.get("token");const tenantId=p.get("tenant_id");if(p.get("autoprint")==="1"){window._qtqdAutoprint=true}if(token&&tenantId&&token!=='undefined'&&tenantId!=='undefined'){const prevTenant=localStorage.getItem("qtqd_tenant_id_v1");if(prevTenant&&prevTenant!==tenantId){localStorage.removeItem(FIELD_CONFIG_KEY)}// Grava diretamente no localStorage (não depende de QTQD_API_CLIENT estar carregado)
localStorage.setItem('qtqd_jwt_v1',token);localStorage.setItem('qtqd_tenant_id_v1',tenantId);localStorage.setItem('qtqd_permissao_v1','edita');if(window.QTQD_API_CLIENT){window.QTQD_API_CLIENT.setJwt(token);window.QTQD_API_CLIENT.setTenantId(tenantId);}history.replaceState(null,"",location.pathname)}})();
initializeClient();
