...
const _dbg=document.createElement('div');_dbg.id='_qtqd_dbg';_dbg.style='position:fixed;top:0;left:0;right:0;background:#1e3a5f;color:#fff;padding:10px;font-size:13px;z-index:99999;font-family:monospace';_dbg.innerHTML='JWT: <b>'+( localStorage.getItem('qtqd_jwt_v1')?'✓ presente':'✗ ausente')+'</b> | Tenant: <b>'+(localStorage.getItem('qtqd_tenant_id_v1')||'ausente')+'</b> | Modo: <b>'+(isApiMode()?'API':'simulação')+'</b> <button onclick="document.getElementById(\'_qtqd_dbg\').remove()" style="margin-left:12px;padding:2px 8px;cursor:pointer">✕</button>';document.body.appendChild(_dbg);
...
(function(){const p=new URLSearchParams(location.search);const token=p.get("token");const tenantId=p.get("tenant_id");if(p.get("autoprint")==="1"){window._qtqdAutoprint=true}if(token&&tenantId&&token!=='undefined'&&tenantId!=='undefined'){localStorage.removeItem(FIELD_CONFIG_KEY);localStorage.setItem('qtqd_jwt_v1',token);localStorage.setItem('qtqd_tenant_id_v1',tenantId);localStorage.setItem('qtqd_permissao_v1','edita');// Recarrega sem params para garantir inicialização limpa com localStorage já gravado
