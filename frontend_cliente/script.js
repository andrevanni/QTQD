...
function createRecordFromValues(v){const qt=parseMoney(v.saldo_bancario)+parseMoney(v.contas_receber)+parseMoney(v.cartoes)+parseMoney(v.convenios)+parseMoney(v.cheques)+parseMoney(v.trade_marketing)+parseMoney(v.outros_qt)+parseMoney(v.estoque_custo);const qd=parseMoney(v.contas_pagar)+parseMoney(v.fornecedores)+parseMoney(v.investimentos_assumidos)+parseMoney(v.outras_despesas_assumidas)+parseMoney(v.dividas)+parseMoney(v.financiamentos)+parseMoney(v.tributos_atrasados)+parseMoney(v.acoes_processos);const saldo=qt-qd;const divs=parseMoney(v.dividas)+parseMoney(v.financiamentos)+parseMoney(v.tributos_atrasados)+parseMoney(v.acoes_processos);const pme=safeDivide(parseMoney(v.estoque_custo)*30,parseMoney(v.venda_custo_mes));const compra=safeDivide(parseMoney(v.contas_pagar)*30,parseMoney(v.compras_mes));const venda=safeDivide(parseMoney(v.contas_receber)*30,parseMoney(v.venda_cupom_mes));const pmpInput=parseMoney(v.pmp);const pmvInput=parseMoney(v.pmv);const pmeExcel=parseMoney(v.pme_excel);const pmeParaCiclo=pmeExcel||pme||0;const ciclo=(pmpInput>0||pmvInput>0)?pmpInput-pmvInput-pmeParaCiclo:null;const excA=parseMoney(v.excesso_curva_a),excB=parseMoney(v.excesso_curva_b),excC=parseMoney(v.excesso_curva_c),excD=parseMoney(v.excesso_curva_d);return {...v,qt_total:qt,qd_total:qd,saldo_qt_qd:saldo,indice_qt_qd:safeDivide(qt,qd),saldo_sem_dividas:saldo+divs,indice_sem_dividas:safeDivide(qt,qd-divs),saldo_sem_dividas_sem_estoque:saldo+divs-parseMoney(v.estoque_custo),prazo_medio_compra:compra,prazo_venda:venda,pme,ciclo_financiamento:ciclo,indice_compra_venda:safeDivide(parseMoney(v.compras_mes),parseMoney(v.venda_custo_mes)),indice_entrada_venda:safeDivide(parseMoney(v.entrada_mes),parseMoney(v.venda_custo_mes)),margem_bruta:safeDivide(parseMoney(v.venda_cupom_mes)-parseMoney(v.venda_custo_mes),parseMoney(v.venda_cupom_mes)),excesso_total:excA+excB+excC+excD}}
...
document.querySelectorAll(".nav-link").forEach(b=>b.addEventListener("click",()=>{clearFeedback();openSection(b.dataset.section)}));
...
// Se tem tenant_id mas não tem JWT → sessão expirada → mostrar login
const hasTenant=!!localStorage.getItem("qtqd_tenant_id_v1");const hasJwt=!!localStorage.getItem("qtqd_jwt_v1");if(hasTenant&&!hasJwt){showLoginScreen();return}
